...
configure_pm2() {
  ...
  pm2 start src/index.js --name verification-bot
  pm2 save
...
main() {
  ...
  configure_firewall
  configure_pm2
  echo "\nVerification Bot installed successfully!"
  echo "Commands:"
  echo "  pm2 status verification-bot"
  echo "  pm2 restart verification-bot"
  echo "  pm2 stop verification-bot"
  echo "  pm2 delete verification-bot"
  echo "  pm2 logs verification-bot"
}
