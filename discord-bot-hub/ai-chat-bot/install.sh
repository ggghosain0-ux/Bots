...
configure_pm2() {
  ...
  pm2 start src/index.js --name ai-chat-bot
  pm2.save
...
main() {
  ...
  configure_firewall
  configure_pm2
  echo "\nAI Chat Bot installed successfully!"
  echo "Commands:"
  echo "  pm2 status ai-chat-bot"
  echo "  pm2 restart ai-chat-bot"
  echo "  pm2 stop ai-chat-bot"
  echo "  pm2 delete ai-chat-bot"
  echo "  pm2 logs ai-chat-bot"
}
